...
public class WalletServlet extends BaseServlet {
	...
	private static final int AuthTypeGoogleAuthenticator = 4;
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {	
			...
			if (guid.equals("faq")) {
				getServletContext().getRequestDispatcher("/WEB-INF/"+ BaseServlet.ROOT + "bitcoin-wallet-faq.jsp").forward(req, res);
				return;
				...
				getServletContext().getRequestDispatcher("/WEB-INF/"+ BaseServlet.ROOT + "bitcoin-wallet-new.jsp").forward(req, res);
				return;
			} else if (guid.equals("paypal-vs-bitcoin")) {
				getServletContext().getRequestDispatcher("/WEB-INF/"+ BaseServlet.ROOT + "bitcoin-vs-paypal.jsp").forward(req, res);
				return;
			} else if (guid.equals("yubikey")) {
				getServletContext().getRequestDispatcher("/WEB-INF/"+ BaseServlet.ROOT + "bitcoin-wallet-yubikey.jsp").forward(req, res);
				return;
			} else if (guid.equals("google-authenticator")) {
				getServletContext().getRequestDispatcher("/WEB-INF/"+ BaseServlet.ROOT + "bitcoin-wallet-google-authenticator.jsp").forward(req, res);
				return;
			} else if (guid.equals("security")) {
				getServletContext().getRequestDispatcher("/WEB-INF/"+ BaseServlet.ROOT + "bitcoin-wallet-security.jsp").forward(req, res);
				return;
			} else if (guid.equals("devices")) {
				getServletContext().getRequestDispatcher("/WEB-INF/"+ BaseServlet.ROOT + "bitcoin-wallet-devices.jsp").forward(req, res);
				return;
			} else if (guid.equals("support-pages")) {
				getServletContext().getRequestDispatcher("/WEB-INF/"+ BaseServlet.ROOT + "bitcoin-wallet-support.jsp").forward(req, res);
				return;
			} else if (guid.equals("paper-tutorial")) {
				getServletContext().getRequestDispatcher("/WEB-INF/"+ BaseServlet.ROOT + "bitcoin-wallet-paper-tutorial.jsp").forward(req, res);
				return;
			} else if (guid.equals("payment-notifications")) {
				getServletContext().getRequestDispatcher("/WEB-INF/"+ BaseServlet.ROOT + "bitcoin-wallet-notifications.jsp").forward(req, res);
				return;
			} else if (guid.equals("backups")) {
				getServletContext().getRequestDispatcher("/WEB-INF/"+ BaseServlet.ROOT + "bitcoin-wallet-backups.jsp").forward(req, res);
				return;
			} else if (guid.equals("anonymity")) {
				getServletContext().getRequestDispatcher("/WEB-INF/"+ BaseServlet.ROOT + "bitcoin-wallet-anonymity.jsp").forward(req, res);
				return;
			} else if (guid.equals("wallet-format")) {
				getServletContext().getRequestDispatcher("/WEB-INF/"+ BaseServlet.ROOT + "bitcoin-wallet-format.jsp").forward(req, res);
				return;
			} else if (guid.equals("escrow")) {
				getServletContext().getRequestDispatcher("/WEB-INF/"+ BaseServlet.ROOT + "bitcoin-wallet-escrow.jsp").forward(req, res);
				return;
			} else if (guid.equals("features")) {
				getServletContext().getRequestDispatcher("/WEB-INF/"+ BaseServlet.ROOT + "bitcoin-wallet-features.jsp").forward(req, res);
				return;
			} else if (guid.equals("features")) {
				getServletContext().getRequestDispatcher("/WEB-INF/"+ BaseServlet.ROOT + "bitcoin-wallet-features.jsp").forward(req, res);
				return;
			} else if (guid.equals("verifier")) {
				getServletContext().getRequestDispatcher("/WEB-INF/"+ BaseServlet.ROOT + "bitcoin-wallet-verifier.jsp").forward(req, res);
				return;
				...
				try {
					...
					if (results.next()) {				
						...
						if (dropbox_enabled == 1) {
							...
							res.getOutputStream().print("Dropbox synchronization not enabled on your account. Enable it in Account Details.");
						}
					} else {
						throw new Exception("Unauthorized");
					}
			...
			if (results.next()) {				
				...
				if (failed_logins >= MaxFailedLogins) {
					if (lockAccount(rguid, email, 240)) {
			...
			RequestLimiter.didRequest(req.getRemoteAddr(), 100);
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			if (method.equals("insert")) {
				...
				PreparedStatement select_smt = conn.prepareStatement("select email, secret_phrase, alias, yubikey, email_verified, http_url, skype_username, dropbox_enabled, google_secret, auth_type from bitcoin_wallets where guid = ? and shared_key = ?");
				...
				try {
					...
					if (results.next()) {
						...
						String google_secret = results.getString(9);
						String google_secret_url = "";
						if (auth_type == AuthTypeGoogleAuthenticator && google_secret != null) {
							if (alias != null) {
				...
				PreparedStatement smt = null;
				try {
					smt = conn.prepareStatement("select payload, email, auth_type, yubikey, acount_locked_time, failed_logins, email_code, google_secret from bitcoin_wallets where guid = ?");
					...
					if (results.next()) {				
						...
						} else if (auth_type == AuthTypeYubikey) {
							String otp = payload;
